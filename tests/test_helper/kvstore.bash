...
function start_kvstore() {
    ...
    run_in_background bazel run $MANY_BAZEL_OPTIONS //src/many-kvstore -- \
        -v \
        $clean \
...
function call_kvstore() {
    ...
    echo "bazel run $MANY_BAZEL_OPTIONS //src/kvstore -- $pem_arg http://localhost:${port}/ $*" >&2
    # `run` doesn't handle empty parameters well, i.e., $pem_arg is empty
    # We need to use `bash -c` to this the issue
    run bash -c "bazel run $MANY_BAZEL_OPTIONS //src/kvstore -- $pem_arg http://localhost:${port}/ $*"
}
